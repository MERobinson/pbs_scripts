...
purpose:
    # Takes chip and input samples, subsamples and re-runs peak calling
required arguments:
    -t|--test : test sample [BAM]
    -c|--control : input/iso control [BAM]
optional arguments:
    ...
    --start : starting value for subsampling (default = 1,000,000)
    --increment : step size for subsampling (default = 1,000,000)
